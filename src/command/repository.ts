...
function repository(moduleName: string, repositoryName: string) {
  ...
  copyFile({
    ...
    dest: pathFolderInterface.replace(
      pathFolderInterface,
      `src/modules/${moduleName}/repository/${repositoryName}.repository.interface.ts`,
    ),
    ignore: '.spec.ts',
    replaceWords: [
      {
  ...
  copyFile({
    ...
    dest: pathFolderImplementation.replace(
      pathFolderImplementation,
      `src/modules/${moduleName}/repository/implementation/${repositoryName}.repository.ts`,
    ),
    ignore: '.spec.ts',
    replaceWords: [
      {
  ...
  copyFile({
    ...
    dest: pathFolderInMemory.replace(
      pathFolderInMemory,
      `src/modules/${moduleName}/repository/in-memory/${repositoryName}.repository.ts`,
    ),
    ignore: '.spec.ts',
    replaceWords: [
      {
