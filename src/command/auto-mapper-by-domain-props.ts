...
export function autoMapperByDomainProps(
  dest: string,
  ...
  const toPersistenceProps = `type ToPersistenceProps = {
  id: ${idType} | null;
...
${params
  ...
  const toDomainMapper = `toDomain(props: ToPersistenceProps): ${item} {
  ...
  const toPersistenceMapper = `toPersistence(item: ${item}): ToPersistenceProps {
  ...
  const mapperData = toPersistence
    ...
    .replace(
      /toPersistence\(item: ([\S\s]*?)\): ToPersistenceProps \{([\S\s]*?)\}/,
      toPersistenceMapper,
    )
    .replace(
      /toDomain\(props: ToPersistenceProps\): ([\S\s]*?) \{([\S\s]*?)\}/,
      toDomainMapper,
    );
